...
// Ruta relativa: el proxy inverso Nginx enruta al microservicio segun el metodo HTTP
...
async function guardarProducto(event) {
    ...
    try {
        let response;
        if (id) { response = await fetch(API_URL + '/' + id, { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify(datos) }); }
        else { response = await fetch(API_URL, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(datos) }); }
